...
export async function captureData(request: FastifyRequest, reply: FastifyReply) {
  try {
    const { periodId } = request.params as { periodId: string };

    // Consultar o banco de dados para obter todos os ratings associados ao período específico
    const ratings = await prisma.rating.findMany({
      where: {
        subject: {
          periodId: periodId, // Filtrar os ratings pelo ID do período
        },
      },
      include: {
        subject: true,
